...
.p-btn-icon { padding:0 10px; min-width:38px; justify-content:center; }
...
<section class="profile-page">
  ...
  <div style="margin-bottom:16px">
    ...
    <h1>My Profile</h1>
    <p class="desc">Manage your personal information and security settings.</p>
  </div>

  <div class="p-header">
    <div class="p-avatar" id="profileAvatarFrame">
      <?php if (!empty($profileAvatar)): ?>
        <img src="<?= $h($profileAvatar) ?>" alt="<?= $h($userName) ?>" id="profileAvatarImg">
      <?php else: ?>
        <span id="profileAvatarInner"><?= $initials ?></span>
      <?php endif; ?>
    </div>
    <div class="p-header-info">
      <p class="p-header-name"><?= $h($userName) ?></p>
  ...
  <div class="p-grid">
    ...
    <div class="p-card">
      <div class="p-card-head"><i data-lucide="user"></i> Personal Information</div>
      <div class="p-card-body">
        ...
        <div class="p-row">
          ...
          <div>
            <label class="p-label">Timezone</label>
            <input class="p-input" type="text" value="<?= $h($userTimezone) ?>">
          </div>
        </div>
        <div class="p-actions">
    ...
    <div class="p-card">
      ...
      <div class="p-card-body">
        ...
        <div id="profilePwStrength" class="p-strength" style="display:none">
          <span class="p-str-bar" id="profileSeg1"></span>
          <span class="p-str-bar" id="profileSeg2"></span>
          <span class="p-str-bar" id="profileSeg3"></span>
          <span class="p-str-bar" id="profileSeg4"></span>
          <span class="p-str-text" id="profileStrengthText">Weak</span>
        </div>
        ...
        <div class="p-actions">
          <button type="button" class="p-btn p-btn-primary" id="btnProfileUpdatePw"><i data-lucide="key"></i> Update Password</button>
        </div>
      </div>
    </div>
  ...
  <div class="p-modal">
    ...
    <div class="p-modal-body">
      <div class="p-modal-tip">
        ...
        <span>This will <strong>permanently deactivate</strong> your admin account. You will be logged out immediately.</span>
      </div>
...
var strText=document.getElementById('profileStrengthText');
var segs=['profileSeg1','profileSeg2','profileSeg3','profileSeg4'];
...
document.getElementById('btnProfileUpdatePw').addEventListener('click',function(){
  ...
  if(!ok)return;
  var btn=document.getElementById('btnProfileUpdatePw');btn.disabled=true;btn.innerHTML='<i data-lucide="loader"></i> Updating…';if(window.lucide)lucide.createIcons();
  fetch('<?= URLROOT ?>/admin/updatePassword',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({current_password:cur,new_password:np,device:navigator.userAgent||'Unknown'})})
...
btnSave.addEventListener('click',function(){
  ...
  fetch('<?= URLROOT ?>/admin/updateProfile',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:(document.getElementById('profileFirstName').value.trim()+' '+document.getElementById('profileLastName').value.trim()).trim(),email:document.getElementById('profileEmail').value.trim(),phone:document.getElementById('profilePhone').value.trim()})})
...
photoIn.addEventListener('change',function(){
  ...
  fetch('<?= URLROOT ?>/admin/uploadProfilePhoto',{method:'POST',body:fd})
...
db.addEventListener('click',function(){db.disabled=true;db.textContent='Deleting…';dmsg.textContent='';
  fetch('<?= URLROOT ?>/admin/deleteAccount',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({password:dp.value})})
...
<!DOCTYPE html>
<html lang="en">
<head><?php $pageTitle = 'My Profile — Admin'; ?>
