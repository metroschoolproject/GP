...
?>
<style>
    .log-page-shell { min-height: 100%; padding: 28px 32px; background: #F4F1EE; font-size: 13.5px; overflow-y: auto; }
    .log-page { --bg:#F4F1EE; --surface:#FFFFFF; --soft:#FFFFFF; --hover:#eddecc; --border:#ead8c7; --border-light:#eddecc; --primary:#6d4c5b; --primary-hover:#7b5c69; --primary-soft:#eddecc; --text:#111827; --muted:#b79c8b; --body:#7b5c69; max-width: 1600px; margin: 0 auto; color: var(--body); font-family: inherit; }
    .log-page * { box-sizing: border-box; }
    .log-header { display: flex; align-items: flex-end; justify-content: space-between; gap: 16px; margin-bottom: 22px; }
    .log-kicker { margin: 0 0 4px; color: var(--muted); font-size: 10px; font-weight: 700; letter-spacing: .12em; text-transform: uppercase; }
    .log-title { margin: 0; color: var(--text); font-family: inherit; font-size: 22px; font-weight: 700; line-height: 1.15; letter-spacing: -.3px; }
    .log-subtitle { max-width: 620px; margin: 8px 0 0; color: var(--body); font-size: 12px; font-weight: 600; line-height: 1.45; }
    .log-export { display: inline-flex; align-items: center; gap: 6px; min-height: 34px; padding: 0 14px; border: 1px solid var(--border); border-radius: .75rem; background: var(--surface); color: var(--primary); font-family: inherit; font-size: 12px; font-weight: 700; text-decoration: none; transition: background .12s; }
    .log-export:hover { background: var(--primary-soft); transform: none; box-shadow: none; }
    ...
    .log-stats { display: grid; grid-template-columns: repeat(4, minmax(0,1fr)); gap: 10px; margin-bottom: 20px; }
    .log-stat { position: relative; min-height: 0; padding: 14px 16px; border: 1px solid var(--border); border-radius: .75rem; background: var(--surface); }
    .log-stat:last-child { border-right: 0; }
    .log-stat::after { content: ""; position: absolute; right: 16px; top: 16px; width: 7px; height: 7px; border-radius: 50%; background: var(--stat-color); box-shadow: 0 0 0 5px color-mix(in srgb, var(--stat-color) 12%, transparent); }
    .log-stat-label { color: var(--muted); font-size: 10px; font-weight: 800; letter-spacing: .1em; text-transform: uppercase; }
    .log-stat-value { display: block; margin-top: 6px; color: var(--text); font-family: inherit; font-size: 20px; font-weight: 700; line-height: 1; font-variant-numeric: tabular-nums; letter-spacing: -.3px; }
    .log-stat-note { display: block; margin-top: 3px; color: var(--muted); font-size: 11px; }

    .log-panel { overflow: hidden; border: 1px solid var(--border); border-radius: .75rem; background: var(--surface); box-shadow: 0 1px 2px rgba(28,25,23,.04); }
    .log-filters { display: grid; grid-template-columns: minmax(240px, 1.6fr) repeat(4, minmax(135px, .7fr)) auto; gap: 8px; padding: 14px 20px; border-bottom: 1px solid var(--border-light); background: var(--surface); }
    .log-search-wrap { position: relative; }
    .log-search-icon { position: absolute; left: 12px; top: 50%; width: 15px; height: 15px; color: var(--muted); transform: translateY(-50%); pointer-events: none; }
    .log-control { width: 100%; min-height: 34px; box-sizing: border-box; border: 1px solid var(--border); border-radius: .75rem; background: var(--surface); padding: 0 10px; color: var(--body); font-family: inherit; font-size: 11px; font-weight: 600; outline: none; }
    .log-control:focus { border-color: var(--primary); box-shadow: 0 0 0 3px rgba(109,76,91,.1); }
    .log-search { padding-left: 39px; }
    .log-control::placeholder { color: var(--muted); }
    .log-filter-actions { display: flex; gap: 8px; }
    .log-apply, .log-reset { display: inline-flex; align-items: center; justify-content: center; min-height: 34px; border-radius: .75rem; padding: 0 14px; font-family: inherit; font-size: 12px; font-weight: 700; cursor: pointer; }
    .log-apply { border: 1px solid var(--primary); background: var(--primary); color: #FFFFFF; }
    .log-reset { border: 1px solid var(--border); background: var(--surface); color: var(--primary); text-decoration: none; }
    .log-reset:hover { background: var(--primary-soft); }

    .log-table-wrap { overflow-x: auto; }
    .log-table { width: 100%; min-width: 940px; border-collapse: collapse; }
    .log-table th { padding: 9px 20px; border-bottom: 1px solid var(--border-light); background: var(--soft); color: var(--muted); font-size: 10px; font-weight: 800; letter-spacing: .1em; text-align: left; text-transform: uppercase; white-space: nowrap; }
    .log-table td { padding: 13px 20px; border-bottom: 1px solid var(--border-light); vertical-align: middle; font-size: 12px; }
    .log-row { position: relative; cursor: pointer; transition: background .14s ease; }
    .log-row:hover, .log-row:focus { background: var(--soft); outline: none; }
    .log-row td:first-child { border-left: 3px solid var(--row-color); }
    .log-time { color: var(--text); font-weight: 700; white-space: nowrap; }
    .log-date { display: block; margin-top: 2px; color: var(--muted); font-size: 10px; font-weight: 700; }
    ...
    .log-event-name { color: var(--text); font-size: 13px; font-weight: 700; }
    ...
    .log-user { color: var(--text); font-size: 13px; font-weight: 700; }
    .log-email { display: block; max-width: 210px; margin-top: 2px; overflow: hidden; color: var(--muted); font-size: 11px; font-weight: 600; text-overflow: ellipsis; white-space: nowrap; }
    ...
    .log-empty-icon { display: inline-flex; width: 52px; height: 52px; align-items: center; justify-content: center; border-radius: 16px; background: #FFFFFF; color: #9b7d89; }
    .log-empty h2 { margin: 15px 0 6px; color: var(--text); font-family: inherit; font-size: 17px; font-weight: 700; }
    .log-empty p { margin: 0; color: #9b7d89; font-size: 12px; }

    .pagination { display: flex; align-items: center; justify-content: space-between; gap: 16px; padding: 15px 18px; background: #FFFFFF; }
    .page-info { color: #9b7d89; font-size: 11px; font-weight: 600; }
    .page-btns { display: flex; align-items: center; gap: 5px; }
    .page-btn { display: inline-flex; width: 30px; height: 30px; align-items: center; justify-content: center; border: 1px solid #e4d2c3; border-radius: .75rem; background: #FFFFFF; color: #7b5c69; font-family: inherit; font-size: 11px; font-weight: 700; text-decoration: none; }
    ...
    .log-drawer { position: fixed; top: 0; right: 0; z-index: 80; width: min(430px, 92vw); height: 100vh; box-sizing: border-box; overflow-y: auto; border-left: 1px solid #ead8c7; background: #FFFFFF; padding: 25px; box-shadow: -20px 0 50px rgba(52,35,43,.13); transform: translateX(105%); transition: transform .24s ease; }
    ...
    .log-drawer-title { margin: 0; color: var(--text); font-family: inherit; font-size: 21px; font-weight: 700; }
    .log-drawer-close { display: inline-flex; width: 36px; height: 36px; flex: 0 0 36px; align-items: center; justify-content: center; border: 1px solid #ead8c7; border-radius: 10px; background: #FFFFFF; color: #7b5c69; cursor: pointer; }
    ...
    .log-detail dd { margin: 6px 0 0; color: #6d4c5b; font-size: 12px; font-weight: 600; line-height: 1.55; overflow-wrap: anywhere; }
    ...
    @media (max-width: 820px) {
        .log-page-shell { padding: 20px 16px; }
