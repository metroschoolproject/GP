<?php
/**
 * SUPPLIER PROFILE — LAYOUT: "Elegant Rose"
 *
 * Based on the admin profile design. Same romantic two-column layout
 * with rose-gold tones, Playfair Display headings, and photo-centric
 * identity card — adapted for supplier context.
 */
...
$statusClass = match (strtolower($status)) {
    'approved', 'verified' => 'bg-app-soft text-app-success',
    'pending' => 'bg-app-surface text-app-warning',
    'rejected', 'banned' => 'bg-app-danger-soft text-app-danger',
    default => 'bg-app-soft text-app-secondary',
};
...
$dashboardContent = function () use (
    ...
    $paymentStatus, $serviceCount, $totalBookings, $avgRating,
...
.identity-card .shop-name {
    font-family: 'Playfair Display', serif;
    font-size: 22px; font-weight: 600; color: var(--ink);
    margin: 0 0 4px;
}
.identity-card .owner-name {
    font-family: 'Poppins', sans-serif;
...
.id-stats { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 10px; margin-top: 18px; padding-top: 18px; border-top: 1px solid var(--border-light); }
.id-stat {
    text-align: center; padding: 12px 8px; border-radius: .75rem;
    background: linear-gradient(135deg, var(--rose-soft), var(--rose-petal));
}
.id-stat .num { font-family: 'Playfair Display', serif; font-size: 22px; font-weight: 700; color: var(--rose-deep); }
...
.profile-msg.success { display: block; background: var(--success-bg); color: var(--success); }
...
<div class="elegant-rose-profile">
    ...
    <div class="profile-grid">
        ...
        <div>
            <div class="card identity-card">
                <div class="card-body">
                    ...
                    <?php if ($shopName): ?>
                        <p class="shop-name"><?= $h($shopName) ?></p>
                    <?php endif; ?>
                    <p class="owner-name"><?= $h($userName) ?></p>
                    <span class="role-tag <?= $statusClass ?>"><?= $h($statusLabel) ?></span>
                    ...
                    <div class="id-meta">
                        ...
                        <div class="meta-row">
                            ...
                            <span class="meta-val"><?= $h($userAddress) ?></span>
                        </div>
                        <?php endif; ?>
                        <div class="meta-row">
                        ...
                        <div class="meta-row">
                            ...
                            <span class="meta-val"><?= $h($categoryNames) ?></span>
                        </div>
                        <?php endif; ?>
                    </div>
                    ...
                    <div class="id-stats">
                        <div class="id-stat">
                            <div class="num"><?= (int)$serviceCount ?></div>
                            <div class="lbl">Services</div>
                        </div>
                        <div class="id-stat">
                            <div class="num"><?= (int)$totalBookings ?></div>
                            <div class="lbl">Bookings</div>
                        </div>
                        <div class="id-stat">
                            <div class="num"><?= $avgRating ? number_format((float)$avgRating, 1) : '-' ?></div>
        ...
        <div style="display:flex;flex-direction:column;gap:20px;">
            ...
            <!-- Card 1: Shop & Personal Information -->
            <div class="card">
                <div class="card-head"><i data-lucide="store"></i> Shop &amp; Personal Information</div>
                <div class="card-body">
                    <div class="form-section">
                        <!-- Inline feedback -->
                        <div id="profileSaveMsg" class="profile-msg"></div>

                        <div class="form-section-title"><i data-lucide="building-2"></i> Business</div>
                        <div class="form-row single">
                            <div>
                                <label class="form-label">Shop Name</label>
                                <input id="profileShopName" class="form-input" type="text" value="<?= $h($shopName) ?>">
                            </div>
                        </div>
                        <div class="form-row single">
                            <div>
                                <label class="form-label">Description</label>
                                <textarea id="profileDescription" class="form-input"><?= $h($description) ?></textarea>
                            </div>
                        </div>
                        <div class="form-row single">
                            <div>
                                <label class="form-label">Business URL</label>
                                <input id="profileBusinessUrl" class="form-input" type="url" value="<?= $h($businessUrl) ?>" placeholder="https://">
                            </div>
                        </div>

                        <div class="form-section-title" style="margin-top:20px;"><i data-lucide="user"></i> Owner</div>
                        <div class="form-row">
                            <div>
                        ...
                        <div class="form-row">
                            ...
                            <div>
                                ...
                                <input id="profileAddress" class="form-input" type="text" value="<?= $h($userAddress) ?>">
                            </div>
                        </div>
                    ...
                    <div class="form-section">
                        ...
                        <div id="profilePwStrength" class="strength-meter" style="display:none;margin-bottom:16px;">
                            <div class="flex items-center justify-between gap-3">
                                ...
                                <span id="strengthText" class="text-[11px] text-[var(--rose)] opacity-70 whitespace-nowrap">Weak</span>
                            </div>
                        </div>
                        ...
                        <div class="form-actions">
                            <button type="button" class="btn btn-primary" id="btnUpdatePw">
...
document.querySelectorAll('.pw-eye').forEach(function(btn) {
    ...
    document.getElementById('btnSaveProfile').addEventListener('click', () => {
        ...
        const businessUrl = document.getElementById('profileBusinessUrl').value.trim();
        ...
        fetch('<?= URLROOT ?>/supplier/updateProfile', {
            method: 'POST',
    ...
    document.getElementById('btnUpdatePw').addEventListener('click', () => {
        ...
        const btnPw = document.getElementById('btnUpdatePw');
        btnPw.disabled = true;
        btnPw.innerHTML = '<i data-lucide="loader"></i> Updating…';
        if (window.lucide) lucide.createIcons();

        fetch('<?= URLROOT ?>/supplier/updatePassword', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
    ...
    photoInput.addEventListener('change', () => {
        ...
        fetch('<?= URLROOT ?>/supplier/uploadProfilePhoto', {
            method: 'POST',
            body: formData,
        })
        .then(res => res.json())
        .then(data => {
            ...
            if (data.ok) {
                ...
                if (!document.getElementById('btnRemovePhoto')) {
                    ...
                    btnPhoto.parentNode.appendChild(rmBtn);
                    if (window.lucide) lucide.createIcons();
                }
            } else {
    ...
    function removePhoto() {
        ...
        fetch('<?= URLROOT ?>/supplier/removeProfilePhoto', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        })
        .then(res => res.json())
