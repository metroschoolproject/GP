...
<div style="max-width:960px;">
  ...
  <header style="background:#fff;border:1px solid #e7e5e4;border-radius:1.2rem;padding:20px;margin-bottom:20px;box-shadow:0 1px 2px rgba(28,25,23,.05);">
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;align-items:center;">
      <div style="background:#fcefe8;border-radius:1rem;padding:20px;">
        ...
        <p style="font-size:11px;color:#78716c;margin-top:4px;">Based on <?= $total ?> completed booking<?= $total !== 1 ? 's' : '' ?>.</p>
        <div style="display:flex;align-items:flex-end;gap:12px;margin-top:16px;">
          <span style="font-size:52px;font-weight:900;color:#673049;line-height:1;"><?= number_format($avg, 1) ?></span>
          <div style="padding-bottom:4px;">
            <div style="color:#d6a72d;font-size:20px;letter-spacing:-1px;"><?= str_repeat('★', (int)round($avg)) . str_repeat('☆', 5 - (int)round($avg)) ?></div>
            <p style="font-size:10px;color:#78716c;margin-top:2px;">out of 5 stars</p>
          </div>
        </div>
      </div>
      <div>
        <p style="font-size:11px;font-weight:700;color:#1c1917;margin-bottom:12px;">Rating Breakdown</p>
        <?php foreach ([5,4,3,2,1] as $star): ?>
          ...
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;font-size:11px;">
            ...
            <div style="flex:1;height:6px;border-radius:999px;background:#e7e5e4;">
              <div style="height:100%;width:<?= $pct ?>%;border-radius:999px;background:<?= $barColors[$star] ?>;"></div>
            </div>
            <span style="width:32px;text-align:right;color:#78716c;"><?= $pct ?>%</span>
          </div>
        <?php endforeach; ?>
      </div>
    </div>
  ...
  <div style="background:#fff;border:1px solid #e7e5e4;border-radius:1.2rem;padding:20px;box-shadow:0 1px 2px rgba(28,25,23,.05);">
    ...
    <?php if (empty($reviews)): ?>
      <p style="font-size:13px;color:#78716c;text-align:center;padding:32px 0;">No reviews yet. Completed bookings from your customers will appear here.</p>
    <?php else: ?>
